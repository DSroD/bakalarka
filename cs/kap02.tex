...
nemění, ve složkách $\dot \matv$ a $\dot \eta$ dojde k refrakci tak, že se po "vložení"\ do metrického tenzoru zachová velikost i znaménko.
\textcolor{red}{přeformulovat lépe?}
...
$\Omega^{\pm}_{\mathrm{i}} = 1 + \frac{1}{6} \eta^{\pm} \bar{\eta}^{\pm}$, ze spojitosti $\eta, \bar{\eta}$ při přechodu přes $\matu=0$
...
rostoucí absolutní hodnotou $b_0$ dochází k větší změně, jak znázorňuje \ref{fig:NullASRing01}.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \begin{tikzpicture}
            \node[inner sep=0pt, anchor=south west] (ds) at (0,0)
        {\adjincludegraphics[trim={{.18\width} {.15\height} {.1\width} {.25\height}}, width=\textwidth, clip]{../img/kap02/ASNullRingxyUmu-1.pdf}};
        \end{tikzpicture}
        \caption{$b_0 = -1$}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \begin{tikzpicture}
            \node[inner sep=0pt, anchor=south west] (ds) at (0,0)
        {\adjincludegraphics[trim={{.18\width} {.15\height} {.1\width} {.25\height}}, width=\textwidth, clip]{../img/kap02/ASNullRingxyUmu-2.pdf}};
        \end{tikzpicture}
        \caption{$b_0 = -2$}
    \end{subfigure}
    \caption{Nulové geodetiky procházející impulsní vlnou Aichelburg-Sexlova řešení. Prstenec testovacích částic je před průchodem impulsem v $\rho=2$.}
    \label{fig:NullASRing01}
\end{figure}

Záporné hodnoty parametru $b_0$, použté v \ref{fig:Null_UV_AichelburgSexl_parameters} a \ref{fig:NullASRing01}, byly vhodné pro přehlednější vizualizaci
efektu impulsní vlny, ale nemají v originální konstrukci pomocí ultraboostu \cite{Aichelburg_1971} fyzikální význam. Parametr $b_0$ totiž v této konstrukci
představuje při limitě ultraboostu $v \to 1, m \to 0$ konstantu, která splňuje $8M = b_0\sqrt{1-v^2}$, pro fyzikální systémy konstruované touto metodou
tedy nabývá kladných hodnot.