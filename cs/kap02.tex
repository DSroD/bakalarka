...
Geometrická metoda konstrukce "cut and paste" impulzních gravitačních vln v Minkowského prostoročase \ref{eq:minkowski} se zakládá na rozdělení celého prostoročasu podél rovinné
...
\ref{eq:lepici_podminky} a nadplocha představuje $\mathcal{U}=0$ plochu konstantní Gaussovské křivosti 
...
\cite{Podolsky:2014ysa}, kde z metriky prostoročasu pozadí \ref{eq:konfmetric}, respektive
\begin{equation}
...
\end{equation}
kde $G(Z, \bar{Z}) \equiv H - Z H_{,Z}-\bar{Z}H_{,\bar{Z}}$. Metriku \ref{eq:nonexp_cont_nokink_metric} pak 
uvažujeme pouze pro $U>0$, zatímco na $U<0$ uvažujeme metriku \ref{eq:null_background_metric}. Definováním
...
\end{equation}
Transformaci \ref{eq:nonexp_cont_transform} lze pomocí Heavisideovy funkce $\Theta(U)$ přepsat do tvaru
spojujícího metriku \ref{eq:null_background_metric} s metrikou \ref{eq:nonexp_continuous_metric}
\begin{equation}
    \label{eq:nonexp_cont_full_transform}
    \mathcal{U}=U,~~~~ \mathcal{V}=V+\Theta(U) H + U_+ H_{,Z}H_{,\bar{Z}},~~~~ \eta=Z+ U_+ H_{,\bar{Z}}.
\end{equation}
Tato transformace obsahuje Penroseovy spojovací podmínky \ref{eq:lepici_podminky} v $U=0$, kde
...
gravtiačních vln s hladkým profilem vlnoplochy k distribučnímu vyjádření impulzní vlny. 
...
\textcolor{red}{Zmínit existenční teorémy (ex. souř. systém kde jsou geodetiky spojité,
